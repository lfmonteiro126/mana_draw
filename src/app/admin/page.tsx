    ...
    return (
      <main className="min-h-screen px-4 py-10 text-[var(--ink)] sm:px-6 lg:px-8">
        <section className="mx-auto max-w-4xl">
          ...
          <div className="card-shadow mt-8 rounded-lg border border-[var(--line)] bg-white p-6">
  ...
  return (
    <main className="min-h-screen text-[var(--ink)]">
      <div className="grid min-h-screen lg:grid-cols-[280px_1fr]">
        <aside className="hidden border-r border-[var(--line)] bg-white/75 backdrop-blur-xl lg:flex lg:flex-col">
          <div className="flex h-[76px] items-center gap-3 border-b border-[var(--line)] px-6">
            <span className="grid h-11 w-11 place-items-center rounded-lg bg-[var(--ink)] text-white">
          ...
          <div className="border-t border-[var(--line)] p-4">
            <Link
              className="flex items-center gap-3 rounded-lg px-3 py-3 text-sm font-medium text-[var(--muted)] transition hover:bg-[#f2efe7] hover:text-[var(--ink)]"
              href="/"
            >
              <ChevronLeft size={18} />
              Voltar para loja
        ...
        <section className="min-w-0">
          <header className="sticky top-0 z-20 border-b border-[var(--line)] bg-[#f7f5f0]/90 backdrop-blur-xl">
            <div className="flex h-[76px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-7">
              ...
              <div className="flex items-center gap-3">
                <Link
                  className="hidden h-11 items-center justify-center rounded-lg border border-[var(--line)] bg-white px-4 text-sm font-semibold text-[var(--ink)] transition hover:border-[var(--accent)] sm:inline-flex"
                  href="/"
                >
                  Loja
                </Link>
                <button className="relative grid h-11 w-11 place-items-center rounded-lg border border-[var(--line)] bg-white text-[var(--muted)]" type="button" aria-label="Notificacoes">
                  <Bell size={18} />
                  {openSubmissions > 0 && (
          ...
          <div className="px-4 py-6 sm:px-6 lg:px-7">
            {(notice || error) && (
              <p className={`mb-5 rounded-lg border px-4 py-3 text-sm ${error ? "border-red-200 bg-red-50 text-red-700" : "border-teal-200 bg-[#e8f5f2] text-[var(--accent-strong)]"}`}>
            ...
            <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.85fr)]">
              <Panel>
                <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  ...
                  <span className="w-fit rounded-lg bg-[#f2efe7] px-3 py-2 text-xs font-semibold text-[var(--muted)]">
                ...
                <form className="mb-4 grid gap-3 lg:grid-cols-[minmax(180px,1fr)_160px_160px_auto]">
                  <label className="relative block">
                    ...
                    <input
                      className="h-11 w-full rounded-lg border border-[var(--line)] bg-white pl-10 pr-3 text-sm text-[var(--ink)] outline-none placeholder:text-[var(--muted)] focus:border-[var(--accent)]"
                      name="query"
                      placeholder="Buscar carta, colecao ou tag"
                      defaultValue={query}
                    />
                  </label>
                  <select className="h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm outline-none focus:border-[var(--accent)]" name="game" defaultValue={game}>
                    <option value="Todos">Todos os jogos</option>
                    {games.map((item) => (
                      <option key={item} value={item}>{item}</option>
                    ))}
                  </select>
                  <select className="h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm outline-none focus:border-[var(--accent)]" name="stock" defaultValue={stock}>
                    <option value="all">Todo estoque</option>
                    <option value="low">Baixo estoque</option>
                    <option value="out">Sem estoque</option>
                  </select>
                  <button className="h-11 rounded-lg bg-[var(--ink)] px-4 text-sm font-bold text-white transition hover:bg-[var(--accent-strong)]" type="submit">
                    Filtrar
                  </button>
                </form>

                <div className="overflow-hidden rounded-lg border border-[var(--line)]">
                  <div className="hidden grid-cols-[minmax(230px,1fr)_145px_105px_135px_105px] gap-4 border-b border-[var(--line)] bg-[#faf9f6] px-4 py-3 text-xs font-semibold uppercase text-[var(--muted)] xl:grid">
                    <span>Carta</span>
                    <span>Preco</span>
                    <span>Estoque</span>
                    <span>Condicao</span>
            ...
            <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
              <Panel>
                ...
                {orders.length === 0 ? (
                  ...
                  <div className="grid gap-3">
                    {orders.map((order) => (
                      <form key={order.id} action={updateOrderStatusAction} className="grid gap-3 rounded-lg border border-[var(--line)] bg-white p-4 sm:grid-cols-[1fr_150px_92px] sm:items-center">
                        ...
                        <button className="h-11 rounded-lg bg-[var(--ink)] px-3 text-sm font-bold text-white transition hover:bg-[var(--accent-strong)]" type="submit">
                          Salvar
                        </button>
                      </form>
                    ))}
              ...
              <Panel>
                <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  ...
                  <span className="w-fit rounded-lg bg-[#f2efe7] px-3 py-2 text-xs font-semibold text-[var(--gold)]">
                ...
                {submissions.length === 0 ? (
                  ...
                  <div className="grid gap-3">
                    {submissions.map((submission) => (
                      <article key={submission.id} className="rounded-lg border border-[var(--line)] bg-white p-4">
                        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                          ...
                          <span className="w-fit rounded-lg bg-[#f2efe7] px-3 py-2 text-xs font-semibold text-[var(--ink)]">
...
function InventoryRow({ card }: { card: TcgCard }) {
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-3">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-[#f2efe7] text-sm font-bold text-[var(--ink)]">
            {card.name.slice(0, 1)}
          </span>
          <div className="min-w-0">
      ...
      <button className="h-11 w-full rounded-lg bg-[var(--ink)] px-3 text-sm font-bold text-white transition hover:bg-[var(--accent-strong)]" type="submit">
        Salvar
      </button>
    </form>
  );
...
function EmptyState({
  ...
  return (
    <div className="rounded-lg border border-dashed border-[var(--line)] bg-[#faf9f6] p-8 text-center">
      <span className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-lg bg-white text-[var(--muted)]">
        {icon}
      </span>
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-lg px-3 py-3 transition ${
        active
          ? "bg-[#f2efe7] text-[var(--ink)] before:-ml-3 before:h-7 before:w-1 before:rounded-r-full before:bg-[var(--accent)]"
          : "hover:bg-[#f2efe7] hover:text-[var(--ink)]"
      }`}
    >
...
function Panel({ children }: { children: ReactNode }) {
  return (
    <div className="card-shadow rounded-lg border border-[var(--line)] bg-white p-5">
      {children}
    </div>
  );
}
...
function MetricCard({
  ...
  const toneClass = {
    cyan: "text-[#0f766e] bg-[#e8f5f2]",
    green: "text-[#134e4a] bg-[#e8f5f2]",
    orange: "text-[var(--gold)] bg-[#f7eddb]",
  ...
  return (
    <div className="card-shadow rounded-lg border border-[var(--line)] bg-white p-5">
      ...
      <div className="flex items-end gap-3">
        ...
        <span className="pb-1 text-sm font-semibold text-[var(--accent-strong)]">{trend}</span>
      </div>
    </div>
  );
}
...
function getGameStats(cards: Array<{ game: Game }>) {
  const items: Array<{ game: Game; barClass: string }> = [
    ...
    { game: "Yu-Gi-Oh!", barClass: "bg-[var(--ink)]" }
  ];
